import type { FilterProject } from "@/container/insights/utils";
import type { LibraryRegion } from "@/types/project/LibraryRegion";
import type { LibraryDivision } from "@/types/project/LibraryDivision";
import type { Contractor } from "@/types/project/Contractor";
import type { FiltersPayload } from "@/container/insights/charts/types";
import { useState, useMemo } from "react";
import Insights from "@/components/layout/insights/Insights";
import ProjectFilters from "@/container/insights/projectFilters/ProjectFilters";
import PortfolioFilters from "@/container/insights/portfolioFilters/PortfolioFilters";
import { projectStatusOptions } from "@/types/project/ProjectStatus";
import ProjectRiskBarChart from "@/container/insights/charts/projectRiskBarChart/ProjectRiskBarChart";
import LocationRiskBarChart from "@/container/insights/charts/locationRiskBarChart/LocationRiskBarChart";
import ProjectRiskHeatmap from "@/container/insights/charts/projectRiskHeatmap/ProjectRiskHeatmap";
import LocationRiskHeatmap from "@/container/insights/charts/locationRiskHeatmap/LocationRiskHeatmap";
import PortfolioTaskRiskHeatmap from "@/container/insights/charts/taskRiskHeatmap/PortfolioTaskRiskHeatmap";
import ProjectTaskRiskHeatmap from "@/container/insights/charts/taskRiskHeatmap/ProjectTaskRiskHeatmap";
import { InsightsMode, InsightsTab } from "@/container/insights/charts/types";
import { useFiltersDescriptions } from "@/container/insights/charts/hooks/useFiltersDescriptions";
import { useTenantStore } from "@/store/tenant/useTenantStore.store";

type PlanningPageProps = {
  projects: FilterProject[];
  regions: LibraryRegion[];
  divisions: LibraryDivision[];
  contractors: Contractor[];
  displayProjectTaskRiskHeatmap?: boolean;
};

export default function PlanningPage({
  projects,
  regions,
  divisions,
  contractors,
  displayProjectTaskRiskHeatmap,
}: PlanningPageProps): JSX.Element {
  const { workPackage, location } = useTenantStore(state =>
    state.getAllEntities()
  );

  const [selectedTab, setSelectedTab] = useState(0);
  const [filters, setFilters] = useState<FiltersPayload>();

  const tab = useMemo(
    () => (selectedTab === 0 ? InsightsTab.PORTFOLIO : InsightsTab.PROJECT),
    [selectedTab]
  );

  const filtersDescriptions = useFiltersDescriptions({
    filters,
    tab,
    descriptions: {
      projectStatusOptions: projectStatusOptions(),
      projects,
      regions,
      divisions,
      contractors,
    },
  });

  const [hasRiskBarData, setHasRiskBarData] = useState(false);
  const [hasProjLocHeatmapData, setHasProjLocHeatmapData] = useState(false);
  const [hasTaskHeatmapData, setHasTaskHeatmapData] = useState(false);

  const projectFilters = (
    <ProjectFilters
      projects={projects}
      timeFrameMode="future"
      onChange={setFilters}
    />
  );

  const portfolioFilters = (
    <PortfolioFilters
      projects={projects}
      projectStatuses={projectStatusOptions()}
      timeFrameMode="future"
      regions={regions}
      divisions={divisions}
      contractors={contractors}
      onChange={setFilters}
    />
  );

  const projectCharts = useMemo(
    () =>
      [
        <LocationRiskBarChart
          key={"LocationRiskBarChart"}
          mode={InsightsMode.PLANNING}
          filters={filters}
          filtersDescriptions={filtersDescriptions}
          setHasData={setHasRiskBarData}
        />,
        <LocationRiskHeatmap
          key="LocationRiskHeatmap"
          filters={filters}
          filtersDescriptions={filtersDescriptions}
          setHasData={setHasProjLocHeatmapData}
        />,
        displayProjectTaskRiskHeatmap && (
          <ProjectTaskRiskHeatmap
            key="ProjectTaskRiskHeatmap"
            filters={filters}
            filtersDescriptions={filtersDescriptions}
            setHasData={setHasTaskHeatmapData}
          />
        ),
      ].filter(comp => !!comp),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [filters]
  );

  const portfolioCharts = useMemo(
    () =>
      [
        <ProjectRiskBarChart
          key="ProjectRiskBarChart"
          mode={InsightsMode.PLANNING}
          filters={filters}
          filtersDescriptions={filtersDescriptions}
          setHasData={setHasRiskBarData}
        />,
        <ProjectRiskHeatmap
          tab={tab}
          key="ProjectRiskHeatmap"
          filters={filters}
          filtersDescriptions={filtersDescriptions}
          setHasData={setHasProjLocHeatmapData}
        />,
        displayProjectTaskRiskHeatmap && (
          <PortfolioTaskRiskHeatmap
            key="PortfolioTaskRiskHeatmap"
            filters={filters}
            filtersDescriptions={filtersDescriptions}
            setHasData={setHasTaskHeatmapData}
          />
        ),
      ].filter(comp => !!comp),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [filters]
  );

  const filterComp =
    tab === InsightsTab.PROJECT ? projectFilters : portfolioFilters;
  const charts = useMemo(
    () => (tab === InsightsTab.PROJECT ? projectCharts : portfolioCharts),
    [tab, projectCharts, portfolioCharts]
  );

  const hasData = hasRiskBarData || hasProjLocHeatmapData || hasTaskHeatmapData;

  return (
    <Insights
      filters={filterComp}
      emptyChartsTitle="There are currently no planning visualizations based on the filters you’ve set"
      emptyChartsDescription={`Try changing your filters to include ${
        tab === InsightsTab.PROJECT
          ? location.labelPlural.toLowerCase()
          : workPackage.labelPlural.toLowerCase()
      } that have data associated with it`}
      charts={charts}
      onTabChange={setSelectedTab}
      hasData={hasData}
    />
  );
}
