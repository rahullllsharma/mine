...
async def test_project_total_task_risk_score_explain_method(
    ...
    data = await ProjectLocationTotalTaskRiskScore.explain(
        metrics_manager, task_manager, location.id, _date
    )
    check_data(data, ["Project Location Total Task Risk Score"])
    ...
    await ProjectLocationTotalTaskRiskScore.store(
        metrics_manager, location.id, _date, 123
    )
    data = await ProjectLocationTotalTaskRiskScore.explain(
        metrics_manager, task_manager, location.id, _date
    )
    ...
    await ProjectLocationTotalTaskRiskScore.store(
        metrics_manager, location.id, _date, 123, inputs=dict(task_ids=[])
    )
    ...
    check_successful_test(data, ["Project Location Total Task Risk Score"], [123])
    check_inputs_errors_length(data, [1], [0])
    check_input(data[0].inputs[0], TaskSpecificRiskScoreModel, dict(value=456))
