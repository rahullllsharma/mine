import datetime

import pytest
from faker import Faker

from tests.db_data import DBData
from tests.factories import LibraryTaskFactory, TaskFactory
from tests.integration.risk_model.explain_functions import (
    check_data,
    check_error,
    check_has_dependencies,
    check_input,
    check_inputs_errors_length,
    check_no_dependencies,
    check_successful_test,
)
from tests.integration.risk_model.utils.common_project_factories import (
    project_with_location_but_no_tasks,
)
from worker_safety_service.dal.risk_model import RiskModelMetricsManager
from worker_safety_service.dal.tasks import TaskManager
from worker_safety_service.models import (
    AsyncSession,
    TaskSpecificRiskScoreModel,
    TaskSpecificSafetyClimateMultiplierModel,
    TaskSpecificSiteConditionsMultiplierModel,
)
from worker_safety_service.risk_model.metrics.tasks.task_specific_riskscore import (
    TaskSpecificRiskScore,
    TaskSpecificRiskScoreInput,
)
from worker_safety_service.risk_model.metrics.tasks.task_specific_safety_climate_multiplier import (
    TaskSpecificSafetyClimateMultiplier,
)
from worker_safety_service.risk_model.metrics.tasks.task_specific_site_conditions_multiplier import (
    TaskSpecificSiteConditionsMultiplier,
)

fake = Faker()


@pytest.mark.asyncio
@pytest.mark.integration
async def test_task_specific_risk_score_explain_method(
    db_session: AsyncSession,
    metrics_manager: RiskModelMetricsManager,
    task_manager: TaskManager,
    db_data: DBData,
) -> None:
    ctx = await project_with_location_but_no_tasks(db_session)
    project = ctx["project"]
    location = (await db_data.project_locations(project.id))[0]

    library_task = await LibraryTaskFactory.persist(
        db_session, name=fake.address(), hesp=123
    )
    task = await TaskFactory.persist(
        db_session, location_id=location.id, library_task_id=library_task.id
    )
    _date = datetime.date.today()
    data = await TaskSpecificRiskScore.explain(
        metrics_manager, task.id, _date, task_manager=task_manager
    )
    check_data(data, ["Task Specific Risk Score"])
    check_inputs_errors_length(data, [0], [1])
    check_error(
        data[0].errors[0],
        TaskSpecificRiskScoreModel,
        {
            "project_task_id": task.id,
            "date": _date,
            "calculated_before": None,
        },
    )
    check_no_dependencies(data)

    await TaskSpecificRiskScore.store(
        metrics_manager,
        task.id,
        _date,
        456,
    )
    data = await TaskSpecificRiskScore.explain(
        metrics_manager, task.id, _date, task_manager=task_manager
    )
    check_data(data, ["Task Specific Risk Score"])
    check_inputs_errors_length(data, [1], [2])
    check_input(
        data[0].inputs[0],
        TaskSpecificRiskScoreInput,
        dict(
            task_hesp_score=123,
            task_specific_site_conditions_multiplier=None,
            task_specific_safety_climate_multiplier=None,
        ),
    )

    if data[0].metric is not None:
        check_error(
            data[0].errors[0],
            TaskSpecificSiteConditionsMultiplierModel,
            {
                "project_task_id": task.id,
                "date": _date,
                "calculated_before": data[0].metric.calculated_at,
            },
        )
        check_error(
            data[0].errors[1],
            TaskSpecificSafetyClimateMultiplierModel,
            {
                "library_task_id": library_task.id,
                "calculated_before": data[0].metric.calculated_at,
            },
        )
    check_no_dependencies(data)

    await TaskSpecificSiteConditionsMultiplier.store(
        metrics_manager, task.id, _date, 789
    )
    data = await TaskSpecificRiskScore.explain(
        metrics_manager, task.id, _date, task_manager=task_manager
    )
    check_inputs_errors_length(data, [1], [2])
    check_input(
        data[0].inputs[0],
        TaskSpecificRiskScoreInput,
        dict(
            task_hesp_score=123,
            task_specific_site_conditions_multiplier=None,
            task_specific_safety_climate_multiplier=None,
        ),
    )

    if data[0].metric is not None:
        check_error(
            data[0].errors[0],
            TaskSpecificSiteConditionsMultiplierModel,
            {
                "project_task_id": task.id,
                "date": _date,
                "calculated_before": data[0].metric.calculated_at,
            },
        )
        check_error(
            data[0].errors[1],
            TaskSpecificSafetyClimateMultiplierModel,
            {
                "library_task_id": library_task.id,
                "calculated_before": data[0].metric.calculated_at,
            },
        )
    check_no_dependencies(data)

    await TaskSpecificSafetyClimateMultiplier.store(
        metrics_manager, library_task.id, 1011, tenant_id=project.tenant_id
    )
    await TaskSpecificRiskScore.store(
        metrics_manager,
        task.id,
        _date,
        456,
    )

    data = await TaskSpecificRiskScore.explain(
        metrics_manager, task.id, _date, task_manager=task_manager
    )
    check_successful_test(data, ["Task Specific Risk Score"], [456])
    check_inputs_errors_length(data, [1], [0])
    check_input(
        data[0].inputs[0],
        TaskSpecificRiskScoreInput,
        dict(
            task_hesp_score=123,
            task_specific_site_conditions_multiplier=789,
            task_specific_safety_climate_multiplier=1011,
        ),
    )
    check_has_dependencies(data[0], 2)

    data = await TaskSpecificRiskScore.explain(
        metrics_manager,
        task.id,
        _date,
        task_manager=task_manager,
        verbose=False,
    )
    check_no_dependencies(data)
