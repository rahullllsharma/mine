...
def migrate() -> None:
    """
    Bring your local db up to date.
    """
    run_cmd("alembic upgrade head", message="Migrating the local db", prefix="MIGRATE")


@app.command()
def migration_downgrade(arg: str = "-1") -> None:
    """
    Downgrade your local database.
    """
    run_cmd(
        f"alembic downgrade {arg}", message="Downgrading the local db", prefix="MIGRATE"
    )


@app.command()
def migration_new(migration_name: str) -> None:
    """
    Create a new migration.
    """
    migrate()
    run_cmd(
        f'alembic revision -m "{migration_name}"',
        message="Creating a new migration",
        prefix="MIGRATE",
    )


@app.command()
def migration_generate(migration_name: str) -> None:
    """
    Generate a new migration.
    """
    migrate()
    run_cmd(
        f'alembic revision -m "{migration_name}" --autogenerate',
        message="Generating a new migration",
        prefix="MIGRATE",
    )
...
def docker() -> None:
    ...
    """
    docker_cmd = "docker-compose up -d postgres graphql keycloak redis"
    run_cmd(docker_cmd, message="Starting up docker containers", prefix="DOCKER")


@app.command()
def docker_graphql_rebuild() -> None:
    """
    Rebuilds the graphql app container.
    """
    docker_cmd = "docker-compose up -d --build graphql"
    run_cmd(docker_cmd, message="Rebuilding graphql container", prefix="DOCKER")
