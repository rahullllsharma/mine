"""update existing form from_id

Revision ID: 9698cecd5c9c
Revises: 33d9764e6f34
Create Date: 2024-07-30 16:46:13.777451
...
revision = "9698cecd5c9c"
down_revision = "33d9764e6f34"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        DECLARE
            ...
            id_exists BOOLEAN;
        BEGIN
            -- Construct the query dynamically to get the last form_id
            query := format('
                SELECT form_id
                FROM %I
                WHERE tenant_id = ''%s''
                ORDER BY created_at DESC, form_id DESC
                LIMIT 1', p_table_name, tenant_id);
            BEGIN
            ...
            -- Check if the generated form_id already exists
            LOOP
                query := format('
                    SELECT EXISTS (
                        SELECT 1
                        ...
                        WHERE tenant_id = ''%s''
                        AND form_id = ''%s''
                    )', p_table_name, tenant_id, new_form_id);

                EXECUTE query INTO id_exists;

                IF NOT id_exists THEN
                    RETURN new_form_id;
                ELSE
                    -- If form_id exists, increment the sequence value and generate a new form_id
                    last_seq_val := (substring(new_form_id from 5)::int);
                    new_form_id := current_month || LPAD((last_seq_val + 1)::TEXT, 5, '0');
                END IF;
            END LOOP;
        END;
    ...
    BEGIN
        LOOP
            ...
            -- Check if new_form_id already exists for the given tenant_id
            query := format('
                SELECT EXISTS (
                    SELECT 1
                    FROM %I
                    WHERE form_id = ''%s''
                      AND tenant_id = ''%s''
                )', p_table_name, new_form_id, p_tenant_id);

            -- Execute the query to check if form_id exists
            EXECUTE query INTO form_id_exists;

            -- If form_id does not exist, break the loop
            EXIT WHEN NOT form_id_exists;
        END LOOP;
