...
class ProjectTotalTaskRiskScore:
    ...
    def __eq__(self, o: object) -> bool:
        if not isinstance(o, ProjectTotalTaskRiskScore):
    ...
    async def run(self) -> None:
        ...
        task_specific_scores: list[float] = []
        task_ids: list[str] = []
        for project_location in project.locations:
            project_tasks = await self._task_manager.get_tasks(
                location_ids=[project_location.id], date=self.date
            )
        ...
        new_value = await self.calc(project.tenant_id, task_specific_scores)
    ...
    async def store(
        ...
        to_store = ProjectTotalTaskRiskScoreModel(
            project_id=project_id, date=date, value=value, inputs=inputs
        )
    ...
    async def explain(
        ...
        inputs: list[TaskSpecificRiskScoreModel] = []
        errors: list[MissingMetricError | MissingDependencyError] = []
        explains: list[list[ExplainMethodOutput]] = []
        try:
            metric = await ProjectTotalTaskRiskScore.load(
                metrics_manager, project_id, date
            )
        except MissingMetricError as e:
            return [ExplainMethodOutput("Project Total Task Risk Score", None, [], [e])]

        task_ids: list[uuid.UUID] = []
        if metric is not None and metric.inputs is not None:
            task_ids = list(map(lambda x: uuid.UUID(x), metric.inputs["task_ids"]))
        for _id in task_ids:
            try:
                inputs.append(
