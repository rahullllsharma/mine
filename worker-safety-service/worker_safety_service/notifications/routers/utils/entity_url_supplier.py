...
def entity_url_supplier(entity_name: str, _id: uuid.UUID | None = None) -> HttpUrl:
    ...
    ret: HttpUrl
    if _id:
        ret = parse_obj_as(HttpUrl, f"{url}/notifications/{entity_name}/{_id}")
    else:  # used for empty sets
        ret = parse_obj_as(HttpUrl, f"{url}/notifications/{entity_name}")
    return ret
...
def entity_array_url_supplier(
    ...
    ret: HttpUrl = parse_obj_as(
        HttpUrl, f"{url}/notifications/{entity_name}?filter[{filter_attribute}]={_id}"
    )
    return ret
